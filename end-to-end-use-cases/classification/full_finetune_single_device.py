...
def decoder_forward(
    ...
    *,
    ...
    # Pooling: use last token hidden state for classification
    pooled = h[:, -1, :]  # shape: [batch, embed_dim]
    # Pass pooled embedding through classification head (self.output)
    logits = self.output(pooled).float()  # shape: [batch, num_classes]
    # Return hidden states + logits if requested, else just logits
    output = logits if not hidden else [*hidden, logits]
    return output
...
class FullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        #print(self._model)
        self._model.output = nn.Linear(4096,2,bias=False).to("cuda").to(torch.bfloat16)
        print(self._model)
    ...
    def _test_step(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    def test(self) -> dict[str, float]:
        """
        Run test loop and get pr curve.
        """
        self._model.eval()
        linspace = np.linspace(start=0.0, stop=1.0, num=1000)

        with torch.no_grad():
            y_true, y_probs = [], []
            for batch_idx, batch in tqdm(enumerate(self._val_dataloader), desc='Test', total=len(self._val_dataloader)):
                

                y_true_batch, y_probs_batch = self._test_step(batch)
